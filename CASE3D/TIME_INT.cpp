...
void TIME_INT(int NNODE, double** GCOORD, int***LNA_3D, int*IEN, int NEL, int TIME, double *T, double DT, int NDT, double* Q, double KAPPA, double PPEAK, double TAU, double XC, double YC,
	...
	*/
	...
	for (z = 0; z < nrbsurfnumber; z++) {
		...
		nr[z].XNRBORG = new double*[elenode2D];
		for (i = 0; i < elenode2D; i++) {
	...
	*/
	...
	if (tfm == 1) {
		...
		if (WAVE == 1) {
			...
			for (z = 0; z < nrbsurfnumber; z++) {
				for (j = 0; j < nr[z].NEL_nrb; j++) {
					for (k = 0; k < elenode2D; k++) {
						nr[z].FEEDOT_inc[nr[z].DP_2D[j*elenode2D + k] - 1][j][0] = FEEDOT[nr[z].IEN_gb[nr[z].DP_2D[j*elenode2D + k] - 1][j] - 1][0];
					}
				}
			}
			for (z = 0; z < nrbsurfnumber; z++) {
				for (j = 0; j < nr[z].NEL_nrb; j++) {
					for (k = 0; k < elenode2D; k++) {
						rd = pow((pow((GCOORD[nr[z].IEN_gb[nr[z].DP_2D[j*elenode2D + k] - 1][j] - 1][0] - XC), 2) + pow((GCOORD[nr[z].IEN_gb[nr[z].DP_2D[j*elenode2D + k] - 1][j] - 1][1] - YC), 2) + pow((GCOORD[nr[z].IEN_gb[nr[z].DP_2D[j*elenode2D + k] - 1][j] - 1][2] - ZC), 2)), 0.5);
						ro = pow((pow((XC - XO), 2) + pow((YC - YO), 2) + pow((ZC - ZO), 2)), 0.5);
						//initialize to 0 
						if (-(rd - ro) / C >= 0) {
							nr[z].XNRBORG[nr[z].DP_2D[j*elenode2D + k] - 1][j] = -(PPEAK*TAU*ro*1.0*(rd - ro + C*TAU - C*TAU*exp(rd / (C*TAU) - ro / (C*TAU)))) / (C*RHO*pow(rd, 2)) - (PPEAK*TAU*ro*1.0*(exp((rd - ro) / (C*TAU)) - 1)) / (C*RHO*rd);
						}
						else {
							nr[z].XNRBORG[nr[z].DP_2D[j*elenode2D + k] - 1][j] = -(PPEAK*TAU*ro*0.0*(rd - ro + C*TAU - C*TAU*exp(rd / (C*TAU) - ro / (C*TAU)))) / (C*RHO*pow(rd, 2)) - (PPEAK*TAU*ro*0.0*(exp((rd - ro) / (C*TAU)) - 1)) / (C*RHO*rd);
						}
	...
	if (element_type == 0) {
		cn = new int[8];
		cn[0] = LNA_3D[0][0][0]; cn[1] = LNA_3D[N][0][0];
		cn[2] = LNA_3D[N][N][0]; cn[3] = LNA_3D[0][N][0];
		cn[4] = LNA_3D[0][0][N]; cn[5] = LNA_3D[N][0][N];
		cn[6] = LNA_3D[N][N][N]; cn[7] = LNA_3D[0][N][N];
		SHG = new double**[4];
		for (i = 0; i < 4; i++) {
		...
		lg = LOCAL_GSHAPE(gq.S, LNA_3D, NINT);
	}

	double* W_new;
	W_new = new double[NINT*NINT*NINT];
	for (int k = 0; k < NINT; k++) {       //k l m are for four points
	...
	for (i = 0; i < NDT; i++) {
		...
		*/
				...
				for (j = 0; j < ol[z].FSNEL; j++) {  //FOR STRUCTURE ELEMENT ON THE FSI BOUNDARY
					for (k = 0; k < elenode2D; k++) {
						if (WAVE == 1) { //plane wave
							...
							ol[z].PSI[ol[z].IEN_lc[ol[z].FP_2D[j*elenode2D + k] - 1][j] - 1]
								= (DT / 2.0)*(WPIN[ol[z].IEN_gb[ol[z].FP_2D[j*elenode2D + k] - 1][j] - 1]); //INCIDENT DISP. PREDICTOR by time integration	
							ol[z].DI[ol[z].IEN_lc[ol[z].FP_2D[j*elenode2D + k] - 1][j] - 1]
								= ol[z].PSI[ol[z].IEN_lc[ol[z].FP_2D[j*elenode2D + k] - 1][j] - 1] / (RHO*C);     //trapezoidal integration of the double integrator
							ol[z].DISPI[ol[z].IEN_lc[ol[z].FP_2D[j*elenode2D + k] - 1][j] - 1][1]
								= ol[z].DISPI[ol[z].IEN_lc[ol[z].FP_2D[j*elenode2D + k] - 1][j] - 1][0] + angle*ol[z].DI[ol[z].IEN_lc[ol[z].FP_2D[j*elenode2D + k] - 1][j] - 1];
						}
						else { //Spherical wave 
							...
							ol[z].PSI[ol[z].IEN_lc[ol[z].FP_2D[j*elenode2D + k] - 1][j] - 1]
								= (DT / 2.0)*(WPIN[ol[z].IEN_gb[ol[z].FP_2D[j*elenode2D + k] - 1][j] - 1]); //INCIDENT DISP. PREDICTOR by time integration	
							PSI_inc[ol[z].IEN_gb[ol[z].FP_2D[j*elenode2D + k] - 1][j] - 1][1] = ol[z].PSI[ol[z].IEN_lc[ol[z].FP_2D[j*elenode2D + k] - 1][j] - 1];
							...
							ol[z].DI[ol[z].IEN_lc[ol[z].FP_2D[j*elenode2D + k] - 1][j] - 1]
								= ol[z].PSI[ol[z].IEN_lc[ol[z].FP_2D[j*elenode2D + k] - 1][j] - 1] / (RHO*C) + (PSI_inc[ol[z].IEN_gb[ol[z].FP_2D[j*elenode2D + k] - 1][j] - 1][0] + PSI_inc[ol[z].IEN_gb[ol[z].FP_2D[j*elenode2D + k] - 1][j] - 1][1]) * (DT / 2) / (RHO * r);  //trapezoidal integration of the double integrator
							ol[z].DISPI[ol[z].IEN_lc[ol[z].FP_2D[j*elenode2D + k] - 1][j] - 1][1]
								= ol[z].DISPI[ol[z].IEN_lc[ol[z].FP_2D[j*elenode2D + k] - 1][j] - 1][0] + angle*ol[z].DI[ol[z].IEN_lc[ol[z].FP_2D[j*elenode2D + k] - 1][j] - 1]; //UPDATED INCIDENT STRUCUTRE DISPLACEMENT 
						}
					}
				}
			}
		...
		if (tfm == 1) { //Total field model
			for (z = 0; z < owsfnumber; z++) {
				for (j = 0; j < ol[z].FSNEL; j++) { //for every fluid element linked to structure
					...
					for (k = 0; k < elenode2D; k++) {
						WBS[ol[z].IEN_gb[ol[z].FP_2D[j*elenode2D + k] - 1][j] - 1] += WBSTEMP[k];
					}
				}
			}
		}
				...
				for (j = 0; j < ol[z].FSNEL; j++) { //for every fluid element linked to structure
					for (h = 0; h < elenode2D; h++) {
						...
						for (k = 0; k < elenode2D_gs; k++) {
							...
							system("PAUSE ");
							WBSTEMP[h] += ol[z].FPMASTER[j][h][k] * (-1) * RHO * (ol[z].DISP_norm[j*elenode2D_gs + k][1] + (ol[z].DISP_norm[j*elenode2D_gs + k][1] - ol[z].DISP_norm[j*elenode2D_gs + k][0]) - ol[z].DISPI[ol[z].IEN_lc[ol[z].FP_2D[j*elenode2D_gs + k] - 1][j] - 1][1]);
						}
					}
					for (k = 0; k < elenode2D; k++) {
		...
		if (improvednrb == 1) {
			//The content below is temporarily commented because we changed the way SHG is defined
			/*
			for (z = 0; z < nrbsurfnumber; z++) {
		...
		if (tfm == 1) { //TFM
			if (WAVE == 1) { //Plane wave
					...
					for (j = 0; j < nr[z].NEL_nrb; j++) { //Need to be changed
						for (k = 0; k < elenode2D; k++) {
							if (improvednrb == 1) {
								...
								if (pow(pow(nr[z].P_dev[j][nr[z].DP_2D[j*elenode2D + k] - 1][0], 2) + pow(nr[z].P_dev[j][nr[z].DP_2D[j*elenode2D + k] - 1][1], 2) + pow(nr[z].P_dev[j][nr[z].DP_2D[j*elenode2D + k] - 1][2], 2), 0.5) != 0) {
									if (debug == 0) {
										nr[z].angle_disp1[nr[z].DP_2D[j*elenode2D + k] - 1][j] = ((-nr[z].P_dev[j][nr[z].DP_2D[j*elenode2D + k] - 1][0]) * (-nr[z].norm[j][0]) + (-nr[z].P_dev[j][nr[z].DP_2D[j*elenode2D + k] - 1][1]) * (-nr[z].norm[j][1]) + (-nr[z].P_dev[j][nr[z].DP_2D[j*elenode2D + k] - 1][2]) * (-nr[z].norm[j][2])) / pow(pow(nr[z].P_dev[j][nr[z].DP_2D[j*elenode2D + k] - 1][0], 2) + pow(nr[z].P_dev[j][nr[z].DP_2D[j*elenode2D + k] - 1][1], 2) + pow(nr[z].P_dev[j][nr[z].DP_2D[j*elenode2D + k] - 1][2], 2), 0.5);
									}
									else {
									...
									nr[z].angle_disp1[nr[z].DP_2D[j*elenode2D + k] - 1][j] = 1.0;
								}
							}
							if (debug == 0) {
							...
							if (debug == 0) {
								nr[z].XEST_ukn[nr[z].DP_2D[j*elenode2D + k] - 1][j] = nr[z].angle_disp1[nr[z].DP_2D[j*elenode2D + k] - 1][j] * (DT / (RHO*C))*P[nr[z].IEN_gb[nr[z].DP_2D[j*elenode2D + k] - 1][j] - 1][0];
							}
							else {
								...
								//std::cout << " " << std::endl;
								nr[z].disp_mag[nr[z].DP_2D[j*elenode2D + k] - 1][j][0] = pow(pow(nr[z].P_dev[j][nr[z].DP_2D[j*elenode2D + k] - 1][0], 2) + pow(nr[z].P_dev[j][nr[z].DP_2D[j*elenode2D + k] - 1][1], 2) + pow(nr[z].P_dev[j][nr[z].DP_2D[j*elenode2D + k] - 1][2], 2), 0.5);
								nr[z].XEST_ukn[nr[z].DP_2D[j*elenode2D + k] - 1][j] = nr[z].angle_disp1[nr[z].DP_2D[j*elenode2D + k] - 1][j] * nr[z].disp_mag[nr[z].DP_2D[j*elenode2D + k] - 1][j][0]; //plane wave approximation (PWA)
							}
						}
					}
					for (j = 0; j < nr[z].NEL_nrb; j++) { //Need to be changed
						for (k = 0; k < elenode2D; k++) {
							nr[z].XNRB_kn[nr[z].DP_2D[j*elenode2D + k] - 1][j][1] = nr[z].XNRB_kn[nr[z].DP_2D[j*elenode2D + k] - 1][j][0] + nr[z].XEST_kn[nr[z].DP_2D[j*elenode2D + k] - 1][j];
							nr[z].XNRB_ukn[nr[z].DP_2D[j*elenode2D + k] - 1][j][1] = nr[z].XNRB_ukn[nr[z].DP_2D[j*elenode2D + k] - 1][j][0] + nr[z].XEST_ukn[nr[z].DP_2D[j*elenode2D + k] - 1][j];
						}
					}
					for (j = 0; j < nr[z].NEL_nrb; j++) {
						for (k = 0; k < elenode2D; k++) {
						...
						for (k = 0; k < elenode2D; k++) {
							BNRB[nr[z].IEN_gb[nr[z].DP_2D[j*elenode2D + k] - 1][j] - 1] += BNRBTEMP[k];
						}
					}
				}
			}
				...
				for (z = 0; z < nrbsurfnumber; z++) { //from bottom to front surface (k=0 means all node case) 
					for (j = 0; j < nr[z].NEL_nrb; j++) { //Need to be changed
						for (k = 0; k < elenode2D; k++) {
							nr[z].FEEDOT_inc[nr[z].DP_2D[j*elenode2D + k] - 1][j][1] = nr[z].FEEDOT_inc[nr[z].DP_2D[j*elenode2D + k] - 1][j][0] + (0.5 * DT)*(PIN[nr[z].IEN_gb[nr[z].DP_2D[j*elenode2D + k] - 1][j] - 1][0] + PIN[nr[z].IEN_gb[nr[z].DP_2D[j*elenode2D + k] - 1][j] - 1][1]);
					...
					for (j = 0; j < nr[z].NEL_nrb; j++) { //Need to be changed
						for (k = 0; k < elenode2D; k++) {
							...
							R = pow(pow(GCOORD[nr[z].IEN_gb[nr[z].DP_2D[j*elenode2D + k] - 1][j] - 1][0] - XC, 2) + pow(GCOORD[nr[z].IEN_gb[nr[z].DP_2D[j*elenode2D + k] - 1][j] - 1][1] - YC, 2) + pow(GCOORD[nr[z].IEN_gb[nr[z].DP_2D[j*elenode2D + k] - 1][j] - 1][2] - ZC, 2), 0.5);
							angle = (nr[z].norm[j][0] * (GCOORD[nr[z].IEN_gb[nr[z].DP_2D[j*elenode2D + k] - 1][j] - 1][0] - XC) + nr[z].norm[j][1] * (GCOORD[nr[z].IEN_gb[nr[z].DP_2D[j*elenode2D + k] - 1][j] - 1][1] - YC) + nr[z].norm[j][2] * (GCOORD[nr[z].IEN_gb[nr[z].DP_2D[j*elenode2D + k] - 1][j] - 1][2] - ZC)) / R;
							if (debug == 0) {
								nr[z].XEST_kn[nr[z].DP_2D[j*elenode2D + k] - 1][j] = angle*((0.5 * DT / (RHO*C))*(PIN[nr[z].IEN_gb[nr[z].DP_2D[j*elenode2D + k] - 1][j] - 1][0] + PIN[nr[z].IEN_gb[nr[z].DP_2D[j*elenode2D + k] - 1][j] - 1][1]) +
									(0.5 * DT / (RHO*R))*(nr[z].FEEDOT_inc[nr[z].DP_2D[j*elenode2D + k] - 1][j][0] + nr[z].FEEDOT_inc[nr[z].DP_2D[j*elenode2D + k] - 1][j][1])) -
									(0.5 * DT / (RHO*C))*(PIN[nr[z].IEN_gb[nr[z].DP_2D[j*elenode2D + k] - 1][j] - 1][0] + PIN[nr[z].IEN_gb[nr[z].DP_2D[j*elenode2D + k] - 1][j] - 1][1]);
							}
							else {
								//nr[z].XEST_kn[nr[z].DP_2D[k] - 1][j] = angle*((0.5 * DT / (RHO*C))*(PIN[nr[z].IEN_gb[nr[z].DP_2D[k] - 1][j] - 1][0] + PIN[nr[z].IEN_gb[nr[z].DP_2D[k] - 1][j] - 1][1]) +
							...
							if (debug == 0) {
								nr[z].XEST_ukn[nr[z].DP_2D[j*elenode2D + k] - 1][j] = nr[z].angle_disp1[nr[z].DP_2D[j*elenode2D + k] - 1][j] * (DT / (RHO*C))*P[nr[z].IEN_gb[nr[z].DP_2D[j*elenode2D + k] - 1][j] - 1][0]; //plane wave approximation (PWA)
							}
							else {
								//nr[z].XEST_ukn[nr[z].DP_2D[k] - 1][j] = nr[z].angle_disp1[nr[z].DP_2D[k] - 1][j] * (DT / (RHO*C))*P[nr[z].IEN_gb[nr[z].DP_2D[k] - 1][j] - 1][0]
									//- nr[z].angle_disp1[nr[z].DP_2D[k] - 1][j] * (0.5 * DT / (RHO*C))*(PIN[nr[z].IEN_gb[nr[z].DP_2D[k] - 1][j] - 1][0] + PIN[nr[z].IEN_gb[nr[z].DP_2D[k] - 1][j] - 1][1]);
					...
					for (j = 0; j < nr[z].NEL_nrb; j++) { //IEN_gb needs to be changed (NINT*N)
						for (h = 0; h < elenode2D; h++) {
							...
							for (k = 0; k < elenode2D; k++) {
								if (debug_algo5 == 0) {
									BNRBTEMP[h] += nr[z].ADMASTER[j][h][k] * (-RHO) * (nr[z].XNRB_kn[nr[z].DP_2D[j*elenode2D + k] - 1][j][1] + nr[z].XNRB_ukn[nr[z].DP_2D[j*elenode2D + k] - 1][j][1] + nr[z].XNRBORG2[nr[z].DP_2D[j*elenode2D + k] - 1][j]);
								}
								else {
			...
			for (z = 0; z < nrbsurfnumber; z++) { //from bottom to front surface (k=0 means all node case)
				for (j = 0; j < nr[z].NEL_nrb; j++) {
					for (k = 0; k < elenode2D; k++) {
						...
						if (debug == 0) {
							nr[z].XEST[nr[z].DP_2D[j*elenode2D + k] - 1][j] = nr[z].angle_disp1[nr[z].DP_2D[j*elenode2D + k] - 1][j] * (DT / (RHO*C))*P[nr[z].IEN_gb[nr[z].DP_2D[j*elenode2D + k] - 1][j] - 1][0];
						}
						else {
							//nr[z].XEST[nr[z].DP_2D[j*elenode2D + k] - 1][j] = nr[z].angle_disp1[nr[z].DP_2D[j*elenode2D + k] - 1][j] * (DT / (RHO*C))*abs(P[nr[z].IEN_gb[nr[z].DP_2D[j*elenode2D + k] - 1][j] - 1][0]);
							nr[z].XEST[nr[z].DP_2D[j*elenode2D + k] - 1][j] = nr[z].angle_disp1[nr[z].DP_2D[j*elenode2D + k] - 1][j] * (DT / (RHO*C))*P[nr[z].IEN_gb[nr[z].DP_2D[j*elenode2D + k] - 1][j] - 1][0];
						}
					}
				}
				for (j = 0; j < nr[z].NEL_nrb; j++) {
					for (k = 0; k < elenode2D; k++) {
						if (debug == 0) {
							nr[z].XNRB[nr[z].DP_2D[j*elenode2D + k] - 1][j][1] = nr[z].XNRB[nr[z].DP_2D[j*elenode2D + k] - 1][j][0] + nr[z].XEST[nr[z].DP_2D[j*elenode2D + k] - 1][j]; //0 IS BEFORE MODIFICATION, 1 IS AFTER MODIFICATION
						}
						else {
							//nr[z].XNRB[nr[z].DP_2D[j*elenode2D + k] - 1][j][1] = nr[z].XNRB[nr[z].DP_2D[j*elenode2D + k] - 1][j][0] + nr[z].XEST[nr[z].DP_2D[j*elenode2D + k] - 1][j]; //0 IS BEFORE MODIFICATION, 1 IS AFTER MODIFICATION
							nr[z].disp_mag[nr[z].DP_2D[j*elenode2D + k] - 1][j][0] = pow(pow(nr[z].P_dev[j][nr[z].DP_2D[j*elenode2D + k] - 1][0], 2) + pow(nr[z].P_dev[j][nr[z].DP_2D[j*elenode2D + k] - 1][1], 2) + pow(nr[z].P_dev[j][nr[z].DP_2D[j*elenode2D + k] - 1][2], 2), 0.5);
							nr[z].XNRB[nr[z].DP_2D[j*elenode2D + k] - 1][j][1] = nr[z].XNRB[nr[z].DP_2D[j*elenode2D + k] - 1][j][0] + DT * nr[z].angle_disp1[nr[z].DP_2D[j*elenode2D + k] - 1][j] * nr[z].disp_mag[nr[z].DP_2D[j*elenode2D + k] - 1][j][0];
							//nr[z].XNRB[nr[z].DP_2D[j*elenode2D + k] - 1][j][2] = nr[z].XNRB[nr[z].DP_2D[j*elenode2D + k] - 1][j][1] + (nr[z].XNRB[nr[z].DP_2D[j*elenode2D + k] - 1][j][1] - nr[z].XNRB[nr[z].DP_2D[j*elenode2D + k] - 1][j][0]); //0 IS BEFORE MODIFICATION, 1 IS AFTER MODIFICATION
						}
					}
				}
			}
			if (WAVE == 1) { //Plane wave
				for (z = 0; z < nrbsurfnumber; z++) {
					for (j = 0; j < nr[z].NEL_nrb; j++) {
						for (k = 0; k < elenode2D; k++) {
							if (debug == 0) {
								NRBDISPTEMP[k] = -RHO* (nr[z].XNRB[nr[z].DP_2D[j*elenode2D + k] - 1][j][1] - nr[z].XNRBORG[nr[z].DP_2D[j*elenode2D + k] - 1][j]);
							}
							else {
								NRBDISPTEMP[k] = -RHO* (nr[z].XNRB[nr[z].DP_2D[j*elenode2D + k] - 1][j][1] - nr[z].XNRBORG[nr[z].DP_2D[j*elenode2D + k] - 1][j]);
							}
				...
				for (z = 0; z < nrbsurfnumber; z++) { //from bottom to front surface (k=0 means all node case)
					...
					for (j = 0; j < nr[z].NEL_nrb; j++) {
						for (h = 0; h < elenode2D; h++) {
							...
							for (k = 0; k < elenode2D; k++) {
								if (debug_algo5 == 0) {
									BNRBTEMP[h] += nr[z].ADMASTER[j][h][k] * (-RHO) * (nr[z].XNRB[nr[z].DP_2D[j*elenode2D + k] - 1][j][1] - nr[z].XNRBORG[nr[z].DP_2D[j*elenode2D + k] - 1][j]);
								}
								else {
				...
				for (j = 0; j < nr[z].NEL_nrb; j++) {
					for (k = 0; k < elenode2D; k++) {
						if (improvednrb == 1) {
							...
							if (pow(pow(nr[z].P_dev[j][nr[z].DP_2D[j*elenode2D + k] - 1][0], 2) + pow(nr[z].P_dev[j][nr[z].DP_2D[j*elenode2D + k] - 1][1], 2) + pow(nr[z].P_dev[j][nr[z].DP_2D[j*elenode2D + k] - 1][2], 2), 0.5) != 0) {
								if (debug == 0) {
									nr[z].angle_disp2[nr[z].DP_2D[j*elenode2D + k] - 1][j] = ((-nr[z].P_dev[j][nr[z].DP_2D[j*elenode2D + k] - 1][0]) * (-nr[z].norm[j][0]) + (-nr[z].P_dev[j][nr[z].DP_2D[j*elenode2D + k] - 1][1]) * (-nr[z].norm[j][1]) + (-nr[z].P_dev[j][nr[z].DP_2D[j*elenode2D + k] - 1][2]) * (-nr[z].norm[j][2])) / pow(pow(nr[z].P_dev[j][nr[z].DP_2D[j*elenode2D + k] - 1][0], 2) + pow(nr[z].P_dev[j][nr[z].DP_2D[j*elenode2D + k] - 1][1], 2) + pow(nr[z].P_dev[j][nr[z].DP_2D[j*elenode2D + k] - 1][2], 2), 0.5);
								}
								else {
									nr[z].angle_disp2[nr[z].DP_2D[j*elenode2D + k] - 1][j] = ((-nr[z].P_dev[j][nr[z].DP_2D[j*elenode2D + k] - 1][0]) * (-nr[z].norm[j][0]) + (-nr[z].P_dev[j][nr[z].DP_2D[j*elenode2D + k] - 1][1]) * (-nr[z].norm[j][1]) + (-nr[z].P_dev[j][nr[z].DP_2D[j*elenode2D + k] - 1][2]) * (-nr[z].norm[j][2])) / pow(pow(nr[z].P_dev[j][nr[z].DP_2D[j*elenode2D + k] - 1][0], 2) + pow(nr[z].P_dev[j][nr[z].DP_2D[j*elenode2D + k] - 1][1], 2) + pow(nr[z].P_dev[j][nr[z].DP_2D[j*elenode2D + k] - 1][2], 2), 0.5);
								}
								...
								nr[z].angle_disp2[nr[z].DP_2D[j*elenode2D + k] - 1][j] = 1.0;
							}
						}
						if (debug == 0) {
							nr[z].XCOR_ukn[nr[z].DP_2D[j*elenode2D + k] - 1][j] = 0.5 * DT * (nr[z].angle_disp1[nr[z].DP_2D[j*elenode2D + k] - 1][j] * P[nr[z].IEN_gb[nr[z].DP_2D[j*elenode2D + k] - 1][j] - 1][0] + nr[z].angle_disp2[nr[z].DP_2D[j*elenode2D + k] - 1][j] * P[nr[z].IEN_gb[nr[z].DP_2D[j*elenode2D + k] - 1][j] - 1][1]) / (RHO*C);
						}
						else {
							//nr[z].XCOR_ukn[nr[z].DP_2D[j*elenode2D + k] - 1][j] = 0.5 * DT * (nr[z].angle_disp1[nr[z].DP_2D[j*elenode2D + k] - 1][j] * P[nr[z].IEN_gb[nr[z].DP_2D[j*elenode2D + k] - 1][j] - 1][0] + nr[z].angle_disp2[nr[z].DP_2D[j*elenode2D + k] - 1][j] * P[nr[z].IEN_gb[nr[z].DP_2D[j*elenode2D + k] - 1][j] - 1][1]) / (RHO*C)
							//- 0.5 * DT * (nr[z].angle_disp1[nr[z].DP_2D[j*elenode2D + k] - 1][j] * PIN[nr[z].IEN_gb[nr[z].DP_2D[j*elenode2D + k] - 1][j] - 1][0] + nr[z].angle_disp2[nr[z].DP_2D[j*elenode2D + k] - 1][j] * PIN[nr[z].IEN_gb[nr[z].DP_2D[j*elenode2D + k] - 1][j] - 1][1]) / (RHO*C);
							//nr[z].XCOR_ukn[nr[z].DP_2D[j*elenode2D + k] - 1][j] = 0.5 * DT * (nr[z].angle_disp1[nr[z].DP_2D[j*elenode2D + k] - 1][j] * P[nr[z].IEN_gb[nr[z].DP_2D[j*elenode2D + k] - 1][j] - 1][0] + nr[z].angle_disp2[nr[z].DP_2D[j*elenode2D + k] - 1][j] * P[nr[z].IEN_gb[nr[z].DP_2D[j*elenode2D + k] - 1][j] - 1][1]) / (RHO*C);
							nr[z].disp_mag[nr[z].DP_2D[j*elenode2D + k] - 1][j][1] = pow(pow(nr[z].P_dev[j][nr[z].DP_2D[j*elenode2D + k] - 1][0], 2) + pow(nr[z].P_dev[j][nr[z].DP_2D[j*elenode2D + k] - 1][1], 2) + pow(nr[z].P_dev[j][nr[z].DP_2D[j*elenode2D + k] - 1][2], 2), 0.5);
							//nr[z].XCOR_ukn[nr[z].DP_2D[j*elenode2D + k] - 1][j] = 0.5 * (nr[z].angle_disp1[nr[z].DP_2D[j*elenode2D + k] - 1][j] * nr[z].disp_mag[nr[z].DP_2D[j*elenode2D + k] - 1][j][0] + nr[z].angle_disp2[nr[z].DP_2D[j*elenode2D + k] - 1][j] * nr[z].disp_mag[nr[z].DP_2D[j*elenode2D + k] - 1][j][1]);
							nr[z].XCOR_ukn[nr[z].DP_2D[j*elenode2D + k] - 1][j] = 0.5 * DT * (nr[z].angle_disp1[nr[z].DP_2D[j*elenode2D + k] - 1][j] * nr[z].disp_mag[nr[z].DP_2D[j*elenode2D + k] - 1][j][0] + nr[z].angle_disp2[nr[z].DP_2D[j*elenode2D + k] - 1][j] * nr[z].disp_mag[nr[z].DP_2D[j*elenode2D + k] - 1][j][1]);
						}
					}
				}
				for (j = 0; j < nr[z].NEL_nrb; j++) { //Need to be changed
					for (k = 0; k < elenode2D; k++) {
						//nr[z].XNRB_ukn[nr[z].DP_2D[j*elenode2D + k] - 1][j][1] = nr[z].XNRB_ukn[nr[z].DP_2D[j*elenode2D + k] - 1][j][0] + XCOR_ukn[nr[z].IEN_gb[nr[z].DP_2D[j*elenode2D + k] - 1][j] - 1];
						nr[z].XNRB_ukn[nr[z].DP_2D[j*elenode2D + k] - 1][j][1] = nr[z].XNRB_ukn[nr[z].DP_2D[j*elenode2D + k] - 1][j][0] + nr[z].XCOR_ukn[nr[z].DP_2D[j*elenode2D + k] - 1][j];
					}
				}
				for (j = 0; j < nr[z].NEL_nrb; j++) { //Need to be changed
					for (k = 0; k < elenode2D; k++) {
						nr[z].XNRB_kn[nr[z].DP_2D[j*elenode2D + k] - 1][j][0] = nr[z].XNRB_kn[nr[z].DP_2D[j*elenode2D + k] - 1][j][1];
						nr[z].XNRB_ukn[nr[z].DP_2D[j*elenode2D + k] - 1][j][0] = nr[z].XNRB_ukn[nr[z].DP_2D[j*elenode2D + k] - 1][j][1];
					}
				}
			}
		}
			...
			for (z = 0; z < nrbsurfnumber; z++) {
				...
				*/
				for (j = 0; j < nr[z].NEL_nrb; j++) {
					for (k = 0; k < elenode2D; k++) {
						...
						if (debug == 0) {
							nr[z].XCOR[nr[z].DP_2D[j*elenode2D + k] - 1][j] = 0.5 * DT * (nr[z].angle_disp1[nr[z].DP_2D[j*elenode2D + k] - 1][j] * P[nr[z].IEN_gb[nr[z].DP_2D[j*elenode2D + k] - 1][j] - 1][0] + nr[z].angle_disp2[nr[z].DP_2D[j*elenode2D + k] - 1][j] * P[nr[z].IEN_gb[nr[z].DP_2D[j*elenode2D + k] - 1][j] - 1][1]) / (RHO*C);
						}
						else {
							//nr[z].XCOR[nr[z].DP_2D[j*elenode2D + k] - 1][j] = 0.5 * DT * (nr[z].angle_disp1[nr[z].DP_2D[j*elenode2D + k] - 1][j] * abs(P[nr[z].IEN_gb[nr[z].DP_2D[j*elenode2D + k] - 1][j] - 1][0]) + nr[z].angle_disp2[nr[z].DP_2D[j*elenode2D + k] - 1][j] * abs(P[nr[z].IEN_gb[nr[z].DP_2D[j*elenode2D + k] - 1][j] - 1][1])) / (RHO*C);
							//nr[z].XCOR[nr[z].DP_2D[j*elenode2D + k] - 1][j] = 0.5 * DT * (nr[z].angle_disp1[nr[z].DP_2D[j*elenode2D + k] - 1][j] * P[nr[z].IEN_gb[nr[z].DP_2D[j*elenode2D + k] - 1][j] - 1][0] + nr[z].angle_disp2[nr[z].DP_2D[j*elenode2D + k] - 1][j] * P[nr[z].IEN_gb[nr[z].DP_2D[j*elenode2D + k] - 1][j] - 1][1]) / (RHO*C);
							nr[z].disp_mag[nr[z].DP_2D[j*elenode2D + k] - 1][j][1] = pow(pow(nr[z].P_dev[j][nr[z].DP_2D[j*elenode2D + k] - 1][0], 2) + pow(nr[z].P_dev[j][nr[z].DP_2D[j*elenode2D + k] - 1][1], 2) + pow(nr[z].P_dev[j][nr[z].DP_2D[j*elenode2D + k] - 1][2], 2), 0.5);
							nr[z].XCOR[nr[z].DP_2D[j*elenode2D + k] - 1][j] = 0.5 * DT * (nr[z].angle_disp1[nr[z].DP_2D[j*elenode2D + k] - 1][j] * nr[z].disp_mag[nr[z].DP_2D[j*elenode2D + k] - 1][j][0] + nr[z].angle_disp2[nr[z].DP_2D[j*elenode2D + k] - 1][j] * nr[z].disp_mag[nr[z].DP_2D[j*elenode2D + k] - 1][j][1]);
						}
					}
				}
				/*
				...
				*/
				for (j = 0; j < nr[z].NEL_nrb; j++) { //Need to be changed
					for (k = 0; k < elenode2D; k++) {
						if (debug == 0) {
							nr[z].XNRB[nr[z].DP_2D[j*elenode2D + k] - 1][j][1] = nr[z].XNRB[nr[z].DP_2D[j*elenode2D + k] - 1][j][0] + nr[z].XCOR[nr[z].DP_2D[j*elenode2D + k] - 1][j];
						}
						else {
							nr[z].XNRB[nr[z].DP_2D[j*elenode2D + k] - 1][j][1] = nr[z].XNRB[nr[z].DP_2D[j*elenode2D + k] - 1][j][0] + nr[z].XCOR[nr[z].DP_2D[j*elenode2D + k] - 1][j];
							//nr[z].disp_mag[nr[z].DP_2D[j*elenode2D + k] - 1][j][1] = pow(pow(nr[z].P_dev[j][nr[z].DP_2D[j*elenode2D + k] - 1][0], 2) + pow(nr[z].P_dev[j][nr[z].DP_2D[j*elenode2D + k] - 1][1], 2) + pow(nr[z].P_dev[j][nr[z].DP_2D[j*elenode2D + k] - 1][2], 2), 0.5);
							//nr[z].XNRB[nr[z].DP_2D[j*elenode2D + k] - 1][j][2] = nr[z].angle_disp2[nr[z].DP_2D[j*elenode2D + k] - 1][j] * nr[z].disp_mag[nr[z].DP_2D[j*elenode2D + k] - 1][j][1];
						}
					}
				}
			}
		...
		if (tfm == 0) {
			for (z = 0; z < nrbsurfnumber; z++) {
				for (j = 0; j < nr[z].NEL_nrb; j++) {
					for (k = 0; k < elenode2D; k++) {
						if (debug == 0) {
							nr[z].XNRB[nr[z].DP_2D[j*elenode2D + k] - 1][j][0] = nr[z].XNRB[nr[z].DP_2D[j*elenode2D + k] - 1][j][1];
						}
						else {
							nr[z].XNRB[nr[z].DP_2D[j*elenode2D + k] - 1][j][0] = nr[z].XNRB[nr[z].DP_2D[j*elenode2D + k] - 1][j][1];
