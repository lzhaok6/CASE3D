...
void readfile(const char* filename) {
	...
	while (!feof(fhandle)) {
		...
		*/
		...
		//Now we know how many elements in the current wetted surface component
		//We need to know nodesperelem in advance (that information should be obtained from Neighborhood_search.cpp)
		if (fptr->dim == 3) {
			fptr->elemnodes = malloc(sizeof(int) * fptr->nelems * 4);
		}
		else {
			fptr->elemnodes = malloc(sizeof(int) * fptr->nelems * 2);
		}
		for (i = 0; i < fptr->nelems; ++i) {
			printf("    %i :", i);
			fscanf(fhandle, "%i", &idx);
			for (n = 0; n < nodesperelem[wetsurfnumber][i]; ++n) {
				fscanf(fhandle, "%i", &fptr->elemnodes[4 * i + n]);
				printf(" %i", fptr->elemnodes[4 * i + n]);
			}
			printf("\n");
			fscanf(fhandle, "\n");
		}
