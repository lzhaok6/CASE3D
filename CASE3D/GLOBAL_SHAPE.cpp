	...
	for (m = 0; m < NEL; m++) {
		for (i = 0; i < NINT*NINT*NINT; i++) {
			for (j = 0; j < NINT; j++) {
				for (k = 0; k < NINT; k++) {
					for (l = 0; l < NINT; l++) {
						t.SHG[m][0][i][j*NINT*NINT + k*NINT + l] = ((XS[m][1 * 3 + 1][j*NINT*NINT + k*NINT + l] * XS[m][2 * 3 + 2][j*NINT*NINT + k*NINT + l] - XS[m][1 * 3 + 2][j*NINT*NINT + k*NINT + l] * XS[m][2 * 3 + 1][j*NINT*NINT + k*NINT + l])*SHL[0][i][j*NINT*NINT + k*NINT + l] + (-XS[m][1 * 3 + 0][j*NINT*NINT + k*NINT + l] * XS[m][2 * 3 + 2][j*NINT*NINT + k*NINT + l] + XS[m][2 * 3 + 0][j*NINT*NINT + k*NINT + l] * XS[m][1 * 3 + 2][j*NINT*NINT + k*NINT + l])*SHL[1][i][j*NINT*NINT + k*NINT + l] + (XS[m][1 * 3 + 0][j*NINT*NINT + k*NINT + l] * XS[m][2 * 3 + 1][j*NINT*NINT + k*NINT + l] - XS[m][2 * 3 + 0][j*NINT*NINT + k*NINT + l] * XS[m][1 * 3 + 1][j*NINT*NINT + k*NINT + l])*SHL[2][i][j*NINT*NINT + k*NINT + l]) / JACOB[m][j*NINT*NINT + k*NINT + l]; //d(phi)/dx
						t.SHG[m][1][i][j*NINT*NINT + k*NINT + l] = ((-XS[m][0 * 3 + 1][j*NINT*NINT + k*NINT + l] * XS[m][2 * 3 + 2][j*NINT*NINT + k*NINT + l] + XS[m][2 * 3 + 1][j*NINT*NINT + k*NINT + l] * XS[m][0 * 3 + 2][j*NINT*NINT + k*NINT + l])*SHL[0][i][j*NINT*NINT + k*NINT + l] + (XS[m][0 * 3 + 0][j*NINT*NINT + k*NINT + l] * XS[m][2 * 3 + 2][j*NINT*NINT + k*NINT + l] - XS[m][2 * 3 + 0][j*NINT*NINT + k*NINT + l] * XS[m][0 * 3 + 2][j*NINT*NINT + k*NINT + l])*SHL[1][i][j*NINT*NINT + k*NINT + l] + (-XS[m][0 * 3 + 0][j*NINT*NINT + k*NINT + l] * XS[m][2 * 3 + 1][j*NINT*NINT + k*NINT + l] + XS[m][2 * 3 + 0][j*NINT*NINT + k*NINT + l] * XS[m][0 * 3 + 1][j*NINT*NINT + k*NINT + l])*SHL[2][i][j*NINT*NINT + k*NINT + l]) / JACOB[m][j*NINT*NINT + k*NINT + l]; //d(phi)/dy
						t.SHG[m][2][i][j*NINT*NINT + k*NINT + l] = ((XS[m][0 * 3 + 1][j*NINT*NINT + k*NINT + l] * XS[m][1 * 3 + 2][j*NINT*NINT + k*NINT + l] - XS[m][1 * 3 + 1][j*NINT*NINT + k*NINT + l] * XS[m][0 * 3 + 2][j*NINT*NINT + k*NINT + l])*SHL[0][i][j*NINT*NINT + k*NINT + l] + (-XS[m][0 * 3 + 0][j*NINT*NINT + k*NINT + l] * XS[m][1 * 3 + 2][j*NINT*NINT + k*NINT + l] + XS[m][1 * 3 + 0][j*NINT*NINT + k*NINT + l] * XS[m][0 * 3 + 2][j*NINT*NINT + k*NINT + l])*SHL[1][i][j*NINT*NINT + k*NINT + l] + (XS[m][0 * 3 + 0][j*NINT*NINT + k*NINT + l] * XS[m][1 * 3 + 1][j*NINT*NINT + k*NINT + l] - XS[m][1 * 3 + 0][j*NINT*NINT + k*NINT + l] * XS[m][0 * 3 + 1][j*NINT*NINT + k*NINT + l])*SHL[2][i][j*NINT*NINT + k*NINT + l]) / JACOB[m][j*NINT*NINT + k*NINT + l]; //d(phi)/dz
						t.SHG[m][3][i][j*NINT*NINT + k*NINT + l] = SHL[3][i][j*NINT*NINT + k*NINT + l]; //phi=phi 
					}
				}
			}
