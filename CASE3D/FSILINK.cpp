						...
						for (n = 0; n < 2; n++) {
							for (l = 0; l < NINT; l++) { //Interpolation points on the original fluid mesh
								for (o = 0; o < NINT; o++) {
									...
									ol[z].FPMASTER_2D[e][ol[z].LNA_algo2[e * 4 + m * 2 + n] - 1][ol[z].LNA_2D[e*NINT*NINT + l*NINT + o] - 1] += FUNC;
								}
							}
						}
					}
			...
			for (e = 0; e < ol[0].FSNEL; e++) {
				for (m = 0; m < 2; m++) { //Interpolation points on the new linear base fluid mesh
					for (n = 0; n < 2; n++) {
						for (l = 0; l < NINT; l++) { //Interpolation points on the original fluid mesh
							for (o = 0; o < NINT; o++) {
								ol[0].dimension_hex[e] += ol[0].FPMASTER_2D[e][ol[0].LNA_algo2[e * 4 + m * 2 + n] - 1][ol[0].LNA_2D[e*NINT*NINT + l*NINT + o] - 1];
							}
						}
					}
				}
